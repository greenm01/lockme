...
static const char *symbol_fragment_source =
	...
	"  float animation_speed = max(rain.r * 2.0, 0.15);\n"
	"  float cycle_rate = max(timing.y, 0.001) * 25.0 * animation_speed;\n"
	"  float phase = randomFloat(cell + vec2(17.0, 19.0));\n"
	"  float bucket = floor((time + phase * 8.0) * cycle_rate);\n"
	"  float symbol_seed = randomFloat(cell + vec2(bucket * 0.37 + 7.0, bucket * 0.11 + 13.0));\n"
	"  float symbol = floor(symbol_seed * glyph_count);\n"
	"  float age = fract(prev.g + cycle_rate * 0.04);\n"
...
static const char *final_fragment_source =
	...
	"  vec2 glyph_uv = vec2((symbol + local.x) / glyph_count, local.y);\n"
