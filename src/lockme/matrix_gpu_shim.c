...
static const char *symbol_fragment_source =
	...
	"  float age = prev.g;\n"
	"  if (frame <= 0.5) {\n"
	"    symbol = floor(randomFloat(cell + vec2(7.0, 13.0)) * glyph_count);\n"
	"    age = randomFloat(cell + vec2(17.0, 19.0));\n"
	"  } else {\n"
	"    float animation_speed = max(rain.r * 2.0, 0.15);\n"
	"    age += animation_speed * max(timing.y, 0.001);\n"
	"    if (age >= 1.0) {\n"
	"      symbol = floor(randomFloat(cell + vec2(frame * 0.37, frame * 0.11)) * glyph_count);\n"
	"      age = fract(age);\n"
	"    }\n"
	"  }\n"
...
static const char *final_fragment_source =
	...
	"  float symbol = floor(clamp(symbol_sample.r, 0.0, 0.99999) * glyph_count);\n"
